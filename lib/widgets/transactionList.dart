      ...
      child: transactions.length > 0 ? ListView.builder(
        itemBuilder: (cxt, index) {
          return Card(
            ...
            trailing: MediaQuery.of(context).size.width > 460 ? FlatButton.icon(icon: Icon(Icons.delete, color:Theme.of(context).errorColor), onPressed: () => deleteTx(transactions[index].id), label: Text('Delete')) : IconButton(icon: Icon(Icons.delete, color:Theme.of(context).errorColor), onPressed: () => deleteTx(transactions[index].id)),
            ),
          );
        },
